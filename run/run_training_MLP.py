...
def main(argv):
    ...
    for seed_group in chunks(cfg.random_seeds, chunk_size):
        ...
        ## eval not full grid
        train_ds_list, x_batches, y_batches, x_eval_batches, y_eval_batches = prep.make_train_and_test_batches(
            p=cfg.p,
        ...
        states, init_metrics = prep.create_states(model, tx, cfg.weight_decay, cfg.batch_size, seed_group)

        # ---- Paths ----
        base = base_dir(cfg.p, mlp_class_lower, cfg.num_neurons, cfg.features, cfg.k)
        mdir = model_dir(cfg.p, cfg.batch_size, cfg.num_neurons, cfg.learning_rate, cfg.weight_decay, cfg.epochs, training_set_size, base, cfg.features)
